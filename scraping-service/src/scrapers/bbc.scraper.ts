...
export class BBCScraper {
  ...
  // 기사 목록 수집
  async collectArticleList(): Promise<
    { title: string; url: string; imageUrl?: string; publishedAt?: string }[]
  > {
    if (!this.page) {
    ...
    try {
      ...
      for (const selector of selectors) {
        ...
        if (articleElements.length > 0) {
          scrapingLogger.info(
            `선택자 "${selector}"로 ${articleElements.length}개 기사 발견`
          );
          break;
        }
      }
  ...
  async scrape(): Promise<ScrapingResult> {
    ...
    try {
      ...
      const articleList = allArticleList.filter((article) =>
        newUrls.includes(article.url)
      );
      ...
      scrapingLogger.info(
        `효율성 - 새로운 기사 ${articleList.length}/${allArticleList.length}개, ${metrics.efficiencyPercentage}% 절약`
      );
      ...
      for (const articleInfo of articleList) {
        ...
        try {
          ...
          // AI 요약 생성
          const [titleSummary, contentSummary, categoryTags] =
            await Promise.all([
          ...
          try {
            ...
            scrapingLogger.info(`처리 및 저장 완료: ${articleDetail.title}`);
          } catch (saveError) {
            const saveErrorMsg = `DB 저장 실패: ${
              articleDetail.originalUrl
            } - ${(saveError as Error).message}`;
            console.log(`  ❌ ${saveErrorMsg}`);
            scrapingLogger.error(saveErrorMsg);
            errors.push(saveErrorMsg);
      ...
      scrapingLogger.info(
        `BBC 스크래핑 완료: ${articles.length}개 기사, ${endTime - startTime}ms`
      );
      return result;
    } catch (error) {
