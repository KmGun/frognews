...
export async function translateTweetToKorean(
  text: string,
  mediaItems?: string[],
  externalLinks?: any[]
): Promise<string | null> {
  try {
    ...
    if (mediaItems && mediaItems.length > 0) {
      ...
      const gifCount = mediaItems.filter(
        (url) => url.includes(".gif") || url.includes("gif")
      ).length;
      ...
      if (mediaParts.length > 0) {
        mediaContext = `\n\n참고: 이 트위터 게시물에는 ${mediaParts.join(
          ", "
        )}가 첨부되어 있습니다.`;
      } else {
        mediaContext = `\n\n참고: 이 트위터 게시물에는 미디어 ${mediaItems.length}개가 첨부되어 있습니다.`;
      }
    }
    ...
    scrapingLogger.info(
      `발견된 링크 수: ${originalLinks.length}개, 미디어: ${
        mediaItems?.length || 0
      }개, 외부링크: ${externalLinks?.length || 0}개`
    );
